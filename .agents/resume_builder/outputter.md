...
4. **Execution Phases** (Execute the phase requested by Conductor):
   - **Phase 1 (HTML & DOCX)**:
     - Run `python .agents/resume_builder/generate_html.py output/resume.md .agents/resume_builder/style.css output/resume.html`
     - Run `python -c "import pypandoc; pypandoc.convert_file('output/resume.md', 'docx', outputfile='output/resume.docx')"`
   - **Phase 2 (PDF)**:
     - Run `python .agents/resume_builder/convert_html_to_pdf.py output/resume.html output/resume.pdf`
